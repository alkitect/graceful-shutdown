...
if [[ -f "${CFG}" ]]; then
  ok "${CFG}"
  for key in POWEROFF_ENABLED DRY_RUN INPUT_IDLE_SEC LOW_LOAD_STREAK_SEC PHASE_LOAD_ENABLED PHASE_A_SEC CPU_PHASE_A_MAX_PCT PHASE_B_WINDOW_SEC CPU_PHASE_B_MAX_PCT GPU_PHASE_B_MAX_PCT CPU_PHASE_B_SPIKE_MAX_PCT CPU_MAX_PCT GPU_MAX_PCT CPU_IDLE_PCT GPU_IDLE_PCT HYSTERESIS_OK_POLLS GRACE_SEC LOAD_WINDOW_ENABLED LOAD_WINDOW_METRIC LOAD_WINDOW_MAX_HIGH NET_CHECK_ENABLED NET_RX_MIN_BPS NET_TX_MIN_BPS NET_BUSY_POLLS BACKUP_CHECK_ENABLED BACKUP_TREAT_UI_PROCESS; do
